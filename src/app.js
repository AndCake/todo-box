...
function sendError(ctx, error) {
	ctx.status = 500;
	ctx.type = 'text/html';
	ctx.body = '<h1>500 Internal Server Error</h1><p>Error during initial component rendering:</p><pre>' + (error.name || 'Unknown error') + ': ' + (error.message || error) + '</pre><p>Reloading in 5s...</p><script>setTimeout(function(){location.reload();}, 5000);</script>';
}
